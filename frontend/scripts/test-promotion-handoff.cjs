...
function main() {
  ...
  try {
    ...
    assert(report.remaining.some((item) => item.includes('acceptance:report:status')), 'handoff should report acceptance preparation')
    assert(report.acceptanceStatus.exists === false, 'handoff should include missing acceptance status')
    assert(report.nextRequiredCommands.includes('npm run acceptance:launch:test'), 'handoff should include launch helper tests')
    assert(report.nextRequiredCommands.includes('npm run acceptance:report:status'), 'handoff should include acceptance status command')
    assert(report.nextRequiredCommands.includes('npm run acceptance:launch'), 'handoff should include acceptance launch command')
  ...
  try {
    ...
    assert(report.remaining.some((item) => item.includes('acceptance:report:status')), 'signed preview handoff should report acceptance preparation')
  ...
  try {
    ...
    assert(report.acceptanceStatus.completionReady === true, 'signed default unpublished handoff should include completed acceptance status')
  ...
  try {
    ...
    assert(report.acceptanceStatus.state === 'accepted', 'signed default handoff should include accepted status guidance')
